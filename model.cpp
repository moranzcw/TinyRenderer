...
Vec3f Model::tangent_normal(Vec2f uvf)
{
    Vec2i uv(uvf[0] * tangentnormalmap_.get_width(), uvf[1] * tangentnormalmap_.get_height());
    TGAColor c = tangentnormalmap_.get(uv[0], uv[1]);
    Vec3f res;
    for (int i = 0; i < 3; i++)
        res[2 - i] = (float)c[i] / 255.f * 2.f - 1.f;
    return res;
